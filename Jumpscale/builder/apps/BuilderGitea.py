...
class BuilderGitea(j.builder.system._BaseClass):
    ...
    def build(self, reset=False):
        ...
        j.sal.process.execute(
            "cd %s && git remote add gigforks https://github.com/gigforks/gitea" % self.GITEAPATH, profile=True
        )
        j.sal.process.execute(
            "cd %s && git fetch gigforks && git checkout gigforks/iyo_integration" % self.GITEAPATH,
            profile=True,
            timeout=1200,
        )
        ...
        j.builder.tools.file_link(source="/sandbox/code/github/incubaid/gitea-custom", destination=self.CUSTOM_PATH)

        self._configure()

        # build gitea (will be stored in self.GITEAPATH/gitea)
        j.sal.process.execute(
            'cd %s && TAGS="bindata" make generate build' % self.GITEAPATH, profile=True, timeout=1200
        )
    ...
    def _configure(self):
        ...
        j.sal.fs.writeFile(location=self.INIPATH, content=config)
