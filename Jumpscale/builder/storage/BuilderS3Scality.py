...
class BuilderS3Scality(j.builder.system._BaseClass):
    ...
    def install(self, start=False, storageLocation="/data/", metaLocation="/meta/"):
        ...
        cmd = "S3DATAPATH={data} S3METADATAPATH={meta} npm start".format(data=storageLocation, meta=metaLocation)
    ...
    def start(self, name=NAME):
        ...
        if self.tools.profile.env_get("NODE_PATH") != nodePath:
            pass
            # self.tools.profile.env_set("NODE_PATH", nodePath)
            # self.tools.profile.path_add(j.builder.tools.replace("{DIR_BASE}/node/bin/"))
