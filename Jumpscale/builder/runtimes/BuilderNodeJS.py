...
class BuilderNodeJS(j.builder.system._BaseClass):
    ...
    def install(self, reset=False):
        ...
        j.sal.process.execute(
            j.core.tools.text_replace("npm config set init-module {DIR_BASE}/node/.npm-init.js -g"), profile=True
        )
        j.sal.process.execute(
            j.core.tools.text_replace("npm config set init-cache {DIR_BASE}/node/.npm -g"), profile=True
        )
        j.sal.process.execute("npm config set global true ", profile=True)
        j.sal.process.execute(
            j.core.tools.text_replace("npm config set init-module {DIR_BASE}/node/.npm-init.js "), profile=True
        )
        j.sal.process.execute(
            j.core.tools.text_replace("npm config set init-cache {DIR_BASE}/node/.npm "), profile=True
        )
