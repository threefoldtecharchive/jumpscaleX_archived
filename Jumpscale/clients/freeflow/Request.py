...
class Request(object):
    ...
    def post(self, path, payload, formatter=None):
        return self.respond(
            self.session.post('{0}{1}'.format(self.base_url, self.ensure_path(path)), json.dumps(payload)),
            formatter)

    def put(self, path, payload, formatter=None):
        return self.respond(
            self.session.put('{0}{1}'.format(self.base_url, self.ensure_path(path)), json.dumps(payload)),
            formatter)

    def patch(self, path, payload, formatter=None):
        return self.respond(
            self.session.patch('{0}{1}'.format(self.base_url, self.ensure_path(path)), json.dumps(payload)),
            formatter)
