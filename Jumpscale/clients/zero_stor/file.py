...
class File:
    ...
    def write(self, key, data):
        ...
        :return: metadata
        """
        return self._stub.Write(model.WriteRequest(key=key, data=data)).metadata
    ...
    def read(self, key):
        ...
        :return: data (bytes)
        """
        return self._stub.Read(model.ReadRequest(key=key)).data
    ...
    def write_file(self, key, file_path):
        ...
        """
        return self._stub.WriteFile(model.WriteFileRequest(key=key, filePath=file_path)).metadata
    ...
    def write_stream(self, key, input, block_size=4096):
        ...
        def stream():
            yield model.WriteStreamRequest(metadata=model.WriteStreamRequest.Metadata(key=key))
            while True:
                chunk = input.read(block_size)
                if len(chunk) == 0:
                    break
                yield model.WriteStreamRequest(data=model.WriteStreamRequest.Data(dataChunk=chunk))
    ...
    def read_stream(self, key, output, chunk_size=4096):
        ...
        response = self._stub.ReadStream(model.ReadStreamRequest(key=key, chunkSize=chunk_size))
    ...
    def delete(self, key):
        ...
        return self._stub.Delete(model.DeleteRequest(key=key))
    ...
    def check(self, key, fast=True):
        ...
        """
        return self._stub.Check(model.CheckRequest(key=key, fast=fast)).status
    ...
    def repair(self, key):
        ...
        return self._stub.Repair(model.RepairRequest(key=key)).metadata
