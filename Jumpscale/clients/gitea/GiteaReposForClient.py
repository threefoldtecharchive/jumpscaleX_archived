...
class GiteaReposForClient(j.application.JSBaseClass):
    ...
    def search(self, query, mode, user_id=None, page_number=1, page_size=150, exclusive=False):
        ...
        items = self.user.client.api.repos.repoSearch(
            exclusive=exclusive, limit=page_size, mode=mode, page=page_number, q=query, uid=user_id
        ).json()
