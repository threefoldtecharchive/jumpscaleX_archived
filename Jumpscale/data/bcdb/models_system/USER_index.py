...
class USER_index:
    ...
    def index_keys_set(self, obj):
        ...
        if val not in ["", None]:
            val = str(val)
            # self._log_debug("key:ipaddr:%s:%s"%(val,obj.id))
            self._index_key_set("ipaddr", val, obj.id)
    ...
    def index_keys_delete(self, obj):
        ...
        if val not in ["", None]:
            val = str(val)
            self._log_debug("delete key:ipaddr:%s:%s" % (val, obj.id))
            self._index_key_delete("ipaddr", val, obj.id)
