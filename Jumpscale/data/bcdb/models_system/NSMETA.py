...
class NSMETA(MODEL_CLASS):
    ...
    def rights_set(self,acl,userids=[],groupids=[],rights="r"):
        ...
        if change:
            ...
            dosave, acl = self._set_pre(acl) #will check if the acl already exists

            if dosave: #in other words did not exist yet
                acl.readonly=True #needs to be on readonly again for next change
                acl.save()
