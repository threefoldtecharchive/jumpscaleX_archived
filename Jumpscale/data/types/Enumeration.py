    ...
    def _data_from_init_val(self, value):
        ...
        if isinstance(value, str):
            ...
            if value_str not in self._typebase.values:
                self._data = 0
                raise j.exceptions.Base("could not find enum:'%s' in '%s'" % (value, self.__repr__()))
            value_id = self._typebase.values.index(value_str) + 1
        elif isinstance(value, int):
            if value > len(self._typebase.values) + 1:
                self._data = 0
                raise j.exceptions.Base("could not find enum id:%s in '%s', too high" % (value, self.__repr__()))
            value_id = value
        else:
            raise j.exceptions.Base("unsupported type for enum, is int or string")
...
class Enumeration(TypeBaseObjFactory):
    ...
    def __init__(self, default=None):
        ...
        if not default:
            raise j.exceptions.Base("enumeration needs default arg to be given e.g. red,blue,... ")
        ...
        if not isinstance(values, list):
            raise j.exceptions.Base("input for enum is comma separated str or list")
