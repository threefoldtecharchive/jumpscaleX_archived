        ...
        if node in self.anchors:
            if self.anchors[node] is None and not self.pyaml_force_embed:
                self.anchors[node] = (
                    ...
                    if not hint
                    else "{}".format(self.pyaml_transliterate("_-_".join(map(op.attrgetter("value"), hint))))
                )
        else:
    ...
    def choose_scalar_style(self):
        ...
        if is_dict_key:
            ...
            if self.event.value.endswith(":"):
                self.event.style = "'"
        return (
            super(UnsafePrettyYAMLDumper, self).choose_scalar_style()
            if self.event.style != "plain"
            else ("'" if " " in self.event.value else None)
        )
...
for str_type in {bytes, unicode}:
    UnsafePrettyYAMLDumper.add_representer(str_type, UnsafePrettyYAMLDumper.represent_stringish)

UnsafePrettyYAMLDumper.add_representer(type(None), lambda s, o: s.represent_scalar("tag:yaml.org,2002:null", ""))
        ...
        for line in buff:
            ...
            if ":" in line:
                ...
                if len(vspacing) > level and len(result) != 1:
                    ...
                    result.insert(-1, vspace if not isinstance(vspace, int) else "\n" * vspace)
        buff.seek(0), buff.truncate()
        buff.write("".join(result).encode("utf-8"))

    def dump(self, data, dst=unicode, safe=False, force_embed=True, vspacing=None, string_val_style=None, **pyyaml_kws):
        buff = io.BytesIO()
        ...
        yaml.dump_all(
            [data], buff, Dumper=Dumper, default_flow_style=False, allow_unicode=True, encoding="utf-8", **pyyaml_kws
        )
