...
class BlockLexer(object):
    ...
    def parse_fences(self, m):
        self.tokens.append({"type": "code", "lang": m.group(2), "text": m.group(3)})

    def parse_heading(self, m):
        self.tokens.append({"type": "heading", "level": len(m.group(1)), "text": m.group(2)})

    def parse_lheading(self, m):
        """Parse setext heading."""
        self.tokens.append({"type": "heading", "level": 1 if m.group(2) == "=" else 2, "text": m.group(1)})
...
class InlineGrammar(object):
    ...
    reflink = re.compile(r"^!?\[(" r"(?:\[[^^\]]*\]|[^\[\]]|\](?=[^\[]*\]))*" r")\]\s*\[([^^\]]*)\]")
    nolink = re.compile(r"^!?\[((?:\[[^\]]*\]|[^\[\]])*)\]")
    url = re.compile(r"""^(https?:\/\/[^\s<]+[^<.,:;"')\]\s])""")
    double_emphasis = re.compile(
        r"^_{2}([\s\S]+?)_{2}(?!_)" r"|" r"^\*{2}([\s\S]+?)\*{2}(?!\*)"  # __word__  # **word**
    )
    emphasis = re.compile(r"^\b_((?:__|[^_])+?)_\b" r"|" r"^\*((?:\*\*|[^\*])+?)\*(?!\*)")  # _word_  # *word*
...
class Renderer(object):
    ...
    def footnote_ref(self, key, index):
        ...
        """
        html = ('<sup class="footnote-ref" id="fnref-%s">' '<a href="#fn-%s">%d</a></sup>') % (
            escape(key),
            escape(key),
            index,
        )
...
class Markdown(object):
    ...
    def output_heading(self):
        return self.renderer.header(self.inline(self.token["text"]), self.token["level"], self.token["text"])
