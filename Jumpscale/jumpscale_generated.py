...
class group_tutorials(JSGroup):
    ...
    @property
    def filestash(self):
        if self._filestash is None:
            from tutorials.tutorials.filestash.Filestash import Filestash
            self._filestash =  Filestash()
        return self._filestash
    @property
    def quake3(self):
        if self._quake3 is None:
            from tutorials.tutorials.quake3.Quake3 import Quake3
            self._quake3 =  Quake3()
        return self._quake3
    @property
    def base(self):
...
class group_clients(JSGroup):
    ...
    def digitalocean(self):
        if self._digitalocean is None:
            from Jumpscale.clients.digitalocean.DigitalOceanFactory import PacketNetFactory
