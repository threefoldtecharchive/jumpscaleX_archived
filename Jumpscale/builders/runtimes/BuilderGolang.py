...
class BuilderGolang(j.builder.system._BaseClass):
    ...
    def install(self, reset=False, old=False):
        ...
        j.builder.core.dir_ensure(self.GOROOTDIR)
        j.builder.core.dir_ensure(self.GOPATHDIR)

        profile = j.builder.sandbox.profileDefault
        profile.envSet("GOROOT", self.GOROOTDIR)
        profile.envSet("GOPATH", self.GOPATHDIR)
        profile.addPath(j.builder.core.joinpaths(self.GOPATHDIR, 'bin'))
        profile.addPath(j.builder.core.joinpaths(self.GOROOTDIR, 'bin'))
        profile.save()

        j.builder.core.file_download(downl, self.GOROOTDIR, overwrite=False, retry=3,
                                       timeout=0, expand=True, removeTopDir=True)

        j.builder.core.dir_ensure("%s/src" % self.GOPATHDIR)
        j.builder.core.dir_ensure("%s/pkg" % self.GOPATHDIR)
        j.builder.core.dir_ensure("%s/bin" % self.GOPATHDIR)
    ...
    def goraml(self, reset=False):
        ...
        sh build.sh
        '''
        j.builder.core.execute_bash(C, profile=True)
        self.doneSet('goraml')
    ...
    def clean_src_path(self):
        srcpath = j.builder.core.joinpaths(self.GOPATHDIR, 'src')
        j.builder.core.dir_remove(srcpath)
