...
class BuilderNodeJS(j.builder.system._BaseClass):
    ...
    def bowerInstall(self, name):
        ...
        if self._bowerDir == "":
            self.install()
            j.builder.core.dir_ensure("{DIR_TEMP}/bower")
    ...
    def phantomjs(self, reset=False):
        ...
        if j.builder.core.isUbuntu:
            ...
            cdest = j.builder.core.file_download(
                url, expand=True, overwrite=False, to="{DIR_TEMP}/phantomjs", removeTopDir=True, deletedest=True)

            j.builder.core.run("mv %s/bin/phantomjs /opt/bin/phantomjs" % cdest)
            j.builder.core.run("rm -rf %s" % cdest)
    ...
    def install(self, reset=False):
        ...
        j.builder.core.file_unlink("{DIR_BIN}/node")
        j.builder.core.dir_remove("{DIR_BASE}/apps/npm")

        # version = "7.7.1"
        version = "8.4.0"

        if reset is False and j.builder.core.file_exists('{DIR_BIN}/npm'):
            return
        ...
        j.builder.core.run("rm -rf {DIR_BASE}/node;mv %s {DIR_BASE}/node" % (cdest))

        # if j.core.platformtype.myplatform.isMac:
        #     j.builder.core.run('mv {DIR_BASE}/node/%s/* {DIR_BASE}/node' %
        ...
        j.builder.core.file_unlink(initmodulepath)
