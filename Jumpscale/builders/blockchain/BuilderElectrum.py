...
class BuilderElectrum(j.builder.system._BaseClass):
    """
    ...
    def build(self, branch=None,tag=None, revision=None, reset=False):
        ...
        path = j.builder.core.joinpaths(j.sal.fs.getTmpDirPath(), 'electrum')
    ...
    def install(self, branch=None,tag="3.2.2", revision=None, reset=False):
        ...
        electrum_bin_path = j.builder.core.joinpaths(base_dir, 'electrum')
        if not j.sal.fs.isFile(electrum_bin_path):
            electrum_bin_path = j.builder.core.joinpaths(electrum_bin_path, 'electrum')

        j.builder.core.dir_ensure( "{DIR_BIN}")
        j.builder.core.file_copy(electrum_bin_path, "{DIR_BIN}/")
    ...
    def start(self, electrum_dir=None, rpcuser='user', rpcpass='pass', rpcport=7777, rpchost='localhost', testnet=False):
        ...
        if electrum_dir is None:
            electrum_dir = j.builder.core.joinpaths(j.dirs.DATADIR, 'electrum')
