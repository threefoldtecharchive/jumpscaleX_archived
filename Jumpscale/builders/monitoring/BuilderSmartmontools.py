...
class BuilderSmartmontools(j.builder.system._BaseClass):
    ...
    def install(self):
        ...
        tmp_location = j.builder.core.file_download(self._DOWNLOAD_URL, expand=True)

        # move downloaded binary to installation destination
        j.builder.core.run("mv %s %s" % (j.sal.fs.joinPaths(tmp_location, self._REL_INSTALL_LOCATION, "smartctl") , j.sal.fs.joinPaths(os.sep, self._REL_INSTALL_LOCATION)))
        
        # cleanup
        j.builder.core.dir_remove(tmp_location)
