...
class BuilderSSH(j.builder.system._BaseClass):
        ...
        if not range:
            ...
            try:
                ...
                if str(e).find("command not found") != -1:
                    ...
                    _, out, _ = j.sal.process.execute("nmap %s -p %s --open -oX {DIR_TEMP}/nmap" %
                                                     (range))
            out = j.builder.core.file_read("{DIR_TEMP}/nmap")
    ...
    def define_host(self, addr, user='root', port=22):
        ...
        lines = j.builder.core.file_read(known_hostsfile, default='').splitlines()
        isknown = False
        for line in lines:
            if line.startswith(addr):
                ...
                break
        if not isknown:
            j.builder.core.execute_bash('ssh-keyscan -p {} -t rsa {} >> {}'.format(port, addr, known_hostsfile))
    ...
    def keygen(self, user="root", keytype="rsa", name="default"):
        ...
        if not j.builder.core.file_exists(path + ".pub"):
            j.builder.core.dir_ensure(home + "/.ssh", mode="0700", owner=user, group=user)

            j.sal.process.execute("ssh-keygen -q -t %s -f %s -N ''" % (keytype, path))
            j.builder.core.file_attribs(path, mode="0600", owner=user, group=user)
            j.builder.core.file_attribs("%s.pub" % path, mode="0600", owner=user, group=user)
            return "%s.pub" % path
        else:
            return "%s.pub" % path
    ...
    def authorize(self, user, key, **kwargs):
        ...
        sudomode = j.builder.core.sudomode
        j.builder.core.sudomode = True
        user = user.strip()
        ...
        if j.builder.core.file_exists(keyf):
            content = j.builder.core.file_read(keyf)
            ...
            j.builder.core.dir_ensure(j.sal.fs.getDirName(keyf), owner=user, group=group, mode='700')
            j.sal.fs.writeFile(keyf, line, owner=user, group=group, mode='600', sudo=False)
            ret = False

        j.builder.core.sudomode = sudomode
        return ret
    ...
    def unauthorize(self, user, key):
        ...
        keyf = d["home"] + "/.ssh/authorized_keys"
        if j.builder.core.file_exists(keyf):
            j.sal.fs.writeFile(keyf, "\n".join(_ for _ in j.builder.core.file_read(keyf).split(
    ...
    def unauthorizeAll(self):
        ...
        j.builder.core.dir_ensure("/root/.ssh")
        j.builder.core.dir_remove("/root/.ssh/known_hosts")
        j.builder.core.dir_remove("/root/.ssh/authorized_keys")
