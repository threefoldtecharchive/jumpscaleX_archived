...
class BuilderGroup(j.builder.system._BaseClass):
    ...
    def create(self, name, gid=None):
        ...
        j.builder.core.sudo("groupadd %s '%s'" % (" ".join(options), name))
    ...
    def ensure(self, name, gid=None):
        ...
        if not d:
            ...
            if gid is not None and d.get("gid") != gid:
                j.builder.core.sudo("groupmod -g %s '%s'" % (gid, name))
    ...
    def user_add(self, group, user):
        ...
        if not self.user_check(group, user):
            j.builder.core.sudo("usermod -a -G '%s' '%s'" % (group, user))
    ...
    def user_del(self, group, user):
        ...
        if self.user_check(group, user):
            ...
            if for_user:
                j.builder.core.sudo("usermod -G '%s' '%s'" % (",".join(for_user), user))
            else:
                j.builder.core.sudo("usermod -G '' '%s'" % (user))
    ...
    def remove(self, group=None, wipe=False):
        ...
        if wipe:
            ...
            if is_primary_group:
                ...
                j.builder.core.sudo("groupdel %s" % group)
