...
class BuilderSSHReflector(j.builder.system._BaseClass):
    ...
    def server(self, reset=False, keyname="reflector"):
        ...
        j.builder.core.dir_ensure("/home/sshreflector/.ssh", recursive=True, mode=None,
                                    owner="sshreflector", group="sshreflector")

        lpath = os.environ["HOME"] + "/.ssh/reflector"
        path = "/home/sshreflector/.ssh/reflector"
        ftp = j.builder.core.executor.sshclient.sftp
        if j.sal.fs.exists(lpath) and j.sal.fs.exists(lpath + ".pub"):
            ...
            if reset or not j.builder.core.file_exists(path) or not j.builder.core.file_exists(path + ".pub"):
                j.builder.core.file_unlink(path)
                j.builder.core.file_unlink(path + ".pub")
                #-N is passphrase
    ...
    def createconnection(self, remoteids):
        ...
        out = j.builder.core.file_read(lpath)
