...
class BuilderUser(j.builder.system._BaseClass):
    ...
    def passwd(self, name, passwd, encrypted_passwd=False):
        ...
        if encrypted_passwd:
            j.builder.core.sudo("usermod -p '%s' %s" % (encoded_password, name))
        else:
            ...
            # j.builder.core.sudo("echo %s | openssl base64 -A -d | chpasswd" % (shell_safe(encoded_password)))
            # j.builder.core.sudo("echo %s | openssl base64 -A -d | chpasswd" % (encoded_password))
    ...
    def create(self, name, passwd=None, home=None, uid=None, gid=None, shell=None,
        ...
        if createhome:
            options.append("-m")
        j.builder.core.sudo("useradd %s '%s'" % (" ".join(options), name))
        if passwd:
    ...
    def check(self, name=None, uid=None, need_passwd=True):
        ...
        need_passwd (Boolean) indicates if password to be included in result or not.
            If set to True it parses 'getent shadow' and needs j.builder.core.sudo access
        """
        ...
        if d:
            ...
            if need_passwd:
                s = j.builder.core.sudo("getent shadow | egrep '^%s:' | awk -F':' '{print $2}'" % (results['name']))
                if s:
                    results['passwd'] = s
        if results:
            return results
    ...
    def ensure(self, name, passwd=None, home=None, uid=None, gid=None,
        ...
        if not d:
            ...
            if fullname is not None and d.get("fullname") != fullname:
                options.append("-c '%s'" % fullname)
            if options:
                j.builder.core.sudo("usermod %s '%s'" % (" ".join(options), name))
            if passwd:
                self.passwd(name=name, passwd=passwd, encrypted_passwd=encrypted_passwd)
    ...
    def remove(self, name, rmhome=None):
        ...
        if rmhome:
            options.append("-r")
        j.builder.core.sudo("userdel %s '%s'" % (" ".join(options), name))

    def list(self):
        users = j.builder.core.find("/home", recursive=False)
