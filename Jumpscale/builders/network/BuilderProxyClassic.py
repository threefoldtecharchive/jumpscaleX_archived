...
class BuilderProxyClassic(j.builder.system._BaseClass):
    ...
    def installCacheProxy(self, storagemntpoint="/storage", btrfs=False):
        ...
        if not j.builder.core.dir_exists(storagemntpoint):
        ...
        if btrfs:
            ...
            j.builder.core.dir_ensure(cachedir)
        ...
        self._logger.info("http://%s:%s/polipo/status?" % (j.builder.core.executor.addr, port))
        self._logger.info(
            "configure your webproxy client to use %s on tcp port %s" %
            (j.builder.core.executor.addr, port))
    ...
    def configureClient(self, addr="", port=8123):
        ...
        if j.builder.prefab.platformtype.myplatform.startswith("ubuntu"):
            f = j.builder.core.file_read("/etc/apt/apt.conf", "")
            f += "\n%s\n" % config
