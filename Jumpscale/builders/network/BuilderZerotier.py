...
class BuilderZerotier(j.builder.system._BaseClass):
    ...
    def build(self, reset=False, install=True):
        ...
        if j.core.platformtype.myplatform.isMac:
            ...
            bindir = '{DIR_BIN}'
            j.builder.core.dir_ensure(bindir)
            for item in ['zerotier-cli', 'zerotier-idtool', 'zerotier-one']:
                j.builder.core.file_copy('{code}/{item}'.format(code=codedir, item=item), bindir+'/')
            return
    ...
    def install(self):
        ...
        for item in j.builder.core.find(j.sal.fs.joinPaths(self.BUILDDIRL, 'usr/sbin')):
            j.builder.core.file_copy(item, bindir + '/')

    def start(self):
        j.builder.sandbox.profileDefault.addPath(j.builder.core.replace("{DIR_BIN}"))
