...
class BuilderZDisk(j.builder.system._BaseClass):
    ...
    def build(self, branch="master", reset=False):
        ...
        j.builder.core.dir_ensure(install_path)
        cmd = """
        cd {0}
        rm -rf ./0-Disk
    ...
    def is_installed(self):
        ...
        return j.builder.core.command_check('nbdserver')\
            and j.builder.core.command_check('tlogserver')\
            and j.builder.core.command_check('zeroctl')
    ...
    tlog_priv_key=None, tlsonly="false"):
        ...
        if config_cluster is not None:
            ...
            j.builder.core.upload(config_file, _NBDSERVER_CONFIG_FILE)
            # set config to file location
            j.builder.core.file_ensure(_NBDSERVER_CONFIG_FILE)
