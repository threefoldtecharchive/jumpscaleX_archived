...
class BuilderS3Scality(j.builder.system._BaseClass):
    ...
    def install(self, start=False, storageLocation="/data/", metaLocation="/meta/"):
        ...
        j.builder.core.dir_ensure(storageLocation)
        j.builder.core.dir_ensure(metaLocation)

        j.builder.core.dir_ensure('/opt/code/github/scality')
        ...
        j.builder.core.dir_remove('{DIR_BASE}/apps/S3', recursive=True)
        j.builder.core.dir_ensure('{DIR_BASE}/apps/')
        ...
        content = j.builder.core.file_read('{DIR_BASE}/apps/S3/package.json')
    ...
    def start(self, name=NAME):
        ...
        j.builder.core.dir_ensure('/data/data')
        j.builder.core.dir_ensure('/data/meta')
        # Temporary. npm install should be added to install() function after updating the building process
        if not j.builder.core.dir_exists('%s/npm-run-all' % nodePath):
            j.sal.process.execute('npm install npm-run-all')
        nodePath = j.builder.core.replace('{DIR_BASE}/node/lib/node_modules/s3/node_modules:%s' % nodePath)
        if j.builder.sandbox.profileDefault.envGet('NODE_PATH') != nodePath:
            j.builder.sandbox.profileDefault.envSet("NODE_PATH", nodePath)
            j.builder.sandbox.profileDefault.addPath(j.builder.core.replace("{DIR_BASE}/node/bin/"))
