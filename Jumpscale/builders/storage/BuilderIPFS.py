...
class BuilderIPFS(j.builder.system._BaseClass):
    ...
    def isInstalled(self):
        ...
        """
        return j.builder.core.file_exists('{DIR_BIN}/ipfs')
    ...
    def install(self, name='main', reset=False):
        ...
        j.builder.core.file_download(url, compress_path)

        uncompress_path = j.core.tools.text_replace('{DIR_TEMP}/go-ipfs')
        if j.builder.core.file_exists(uncompress_path):
            j.builder.core.dir_remove(uncompress_path)

        j.sal.process.execute("cd {DIR_TEMP}; tar xvf {}".format(name))
        j.builder.core.file_copy('{}/ipfs'.format(uncompress_path), '{DIR_BIN}/ipfs')

    def uninstall(self):
        """
        remove ipfs binary from {DIR_BIN}
        """
        if j.builder.core.file_exists('{DIR_BIN}/ipfs'):
            j.builder.core.file_unlink('{DIR_BIN}/ipfs')

    def start(self, name='main', readonly=False):
        cfg_dir = '{DIR_BASE}/cfg/ipfs/{}'.format(name)
        if not j.builder.core.file_exists(cfg_dir):
            j.builder.core.dir_ensure(cfg_dir)

        # check if the ipfs repo has not been created yet.
        if not j.builder.core.file_exists(cfg_dir + '/config'):
