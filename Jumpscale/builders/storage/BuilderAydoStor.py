...
class BuilderAydoStor(j.builder.system._BaseClass):
    ...
    def build(self, addr='0.0.0.0:8090', backend="{DIR_VAR}/aydostor", start=True, install=True, reset=False):
        ...
        j.builder.core.dir_remove("%s/src" % j.builder.sandbox.envGet('GOPATH'))
    ...
    def install(self, addr='0.0.0.0:8090', backend="{DIR_VAR}/aydostor", start=True):
        ...
        j.builder.core.file_copy(j.builder.core.joinpaths(
            j.builder.core.dir_paths['GODIR'], 'bin', 'stor'), '{DIR_BIN}', overwrite=True)
        j.builder.sandbox.addPath("{DIR_BASE}/bin")

        pm = j.builder.system.processmanager.get()
        pm.stop("stor")  # will also kill

        j.builder.core.dir_ensure("{DIR_BASE}/cfg/stor")
        backend = j.core.tools.text_replace(backend)
        j.builder.core.dir_ensure(backend)
        config = {
            'listen_addr': addr,
            'store_root': backend,
        }
        content = j.data.serializers.toml.dumps(config)
        j.builder.core.dir_ensure('{DIR_VAR}/templates/cfg/stor', recursive=True)
    ...
    def start(self, addr):
        ...
        j.builder.core.dir_ensure("{DIR_BASE}/cfg/stor/", recursive=True)
        j.builder.core.file_copy("{DIR_VAR}/templates/cfg/stor/config.toml", "{DIR_BASE}/cfg/stor/")
