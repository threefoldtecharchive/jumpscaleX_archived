...
class BuilderRestic(j.builder.system._BaseClass):
    ...
    def quick_install(self, install=True, reset=False):
        if reset is False and (self.isInstalled() or self.doneGet('quick_install')):
            return
        if not j.builder.core.file_exists(self.DOWNLOAD_DEST):
            j.builder.core.file_download('https://github.com/restic/restic/releases/download/v0.9.0/restic_0.9.0_linux_amd64.bz2', self.DOWNLOAD_DEST)
        j.builder.core.file_expand(self.DOWNLOAD_DEST)
    ...
    def install(self, source=None, reset=False):
        ...
        if source:
            j.builder.core.file_copy(self.FILE_NAME, '{DIR_BIN}/restic' )
        else:
            j.builder.core.file_copy(self.CODEDIR + '/restic', '{DIR_BIN}')
