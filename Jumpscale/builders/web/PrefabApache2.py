...
class BuilderApache2(j.builder.system._BaseClass):
    ...
    def build(self, reset=True):
        ...
        if reset and j.builder.core.dir_exists(httpdir):
            j.builder.core.dir_remove("{DIR_BASE}/apps/apache2")

        j.builder.core.dir_ensure("/optvar/build")

        # DOWNLOAD LINK
        DOWNLOADLINK = 'www-eu.apache.org/dist//httpd/httpd-2.4.29.tar.bz2'
        dest = j.sal.fs.joinPaths("/optvar", 'httpd-2.4.29.tar.bz2')

        if not j.builder.core.file_exists(dest):
            j.builder.core.file_download(DOWNLOADLINK, dest)

        # EXTRACT SROURCE CODE
        j.sal.process.execute(
            "cd /optvar/build && tar xjf {dest} && cp -r /optvar/build/httpd-2.4.29 /optvar/build/httpd".format(**locals()))
        j.builder.core.dir_ensure("{DIR_BASE}/apps/apache2/bin")
        j.builder.core.dir_ensure("{DIR_BASE}/apps/apache2/lib")
    ...
    def install(self):
        ...
        j.builder.core.file_copy("{DIR_BASE}/apps/apache2/bin/*", '{DIR_BIN}/')

    def configure(self):
        conffile = j.builder.core.file_read("{DIR_BASE}/apps/apache2/conf/httpd.conf")
        # SANE CONFIGURATIONS
        lines = """
        ...
        for line in disabled.splitlines():
            ...
            if line:
                mod = "#" + line
                conffile = conffile.replace(line, mod)
        sitesdirconf = j.builder.core.replace("\nInclude %s/apache2/sites-enabled/*" % j.dirs.CFGDIR)
        conffile += sitesdirconf
        conffile += "\nAddType application/x-httpd-php .php"

        # MAKE VHOSTS DIRECTORY
        j.builder.core.dir_ensure("%s/apache2/sites-enabled/" % j.dirs.CFGDIR)
        j.builder.core.dir_ensure("{DIR_BASE}/apps/apache2/sites-available")
        j.builder.core.dir_ensure("{DIR_BASE}/apps/apache2/sites-enabled")
