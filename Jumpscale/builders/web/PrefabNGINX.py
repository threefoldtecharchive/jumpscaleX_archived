...
class BuilderNGINX(j.builder.system._BaseClass):
    ...
    def install(self, start=True):
        ...
        # j.builder.core.dir_ensure("{DIR_BASE}/apps/nginx/")
        # j.builder.core.dir_ensure("{DIR_BASE}/apps/nginx/bin")
        # j.builder.core.dir_ensure("{DIR_BASE}/apps/nginx/etc")
        j.builder.core.dir_ensure("{DIR_BASE}/cfg")
        j.builder.core.dir_ensure("{DIR_TEMP}")
        # j.builder.core.dir_ensure("/optvar/tmp")
        j.builder.core.dir_ensure("{DIR_BASE}/apps/nginx/")
        j.builder.core.dir_ensure("{DIR_BASE}/apps/nginx/bin")
        j.builder.core.dir_ensure("{DIR_BASE}/apps/nginx/etc")
        j.builder.core.dir_ensure("{DIR_BASE}/cfg/nginx/etc")

        j.builder.core.file_copy('/usr/sbin/nginx', '{DIR_BASE}/apps/nginx/bin/nginx', overwrite=True)
        j.builder.core.dir_ensure('/var/log/nginx')
        j.builder.core.file_copy('/etc/nginx/*', '{DIR_BASE}/apps/nginx/etc/', recursive=True)  # default conf
        j.builder.core.file_copy('/etc/nginx/*', '{DIR_BASE}/cfg/nginx/etc/', recursive=True)  # variable conf
        """
        ...
        j.builder.core.dir_ensure("{DIR_VAR}/build/nginx/conf/conf.d/")
        j.builder.core.dir_ensure("{DIR_VAR}/build/nginx/conf/sites-enabled/")
        ...
        fst_cgi_conf = j.builder.core.file_read("{DIR_VAR}/build/nginx/conf/fastcgi.conf")
        ...
        #j.builder.core.file_link(source="{DIR_BASE}/cfg/nginx", destination="{DIR_BASE}/apps/nginx")
        if start:
            self.start()
    ...
    def build(self, install=True):
        ...
        if j.builder.core.isUbuntu:
            ...
            j.builder.core.dir_remove("{DIR_TEMP}/build/nginx")
            j.builder.core.dir_ensure("{DIR_TEMP}/build/nginx")
    ...
    def start(self, name="nginx", nodaemon=True, nginxconfpath=None):
        ...
        if j.builder.core.file_exists(nginxconfpath):
            # foreground
