...
class BuilderARDB(j.builder.system._BaseClass):
    ...
    def build(self, reset=False, storageEngine="forestdb"):
        ...
        j.builder.core.execute_bash(j.core.tools.text_replace(C))
    ...
    def install(self, name='main', host='localhost', port=16379, datadir=None, reset=False, start=True):
        ...
        j.builder.core.dir_ensure("$CFGDIR")
        if not j.builder.core.file_exists('{DIR_BIN}/ardb-server'):
            j.builder.core.file_copy("{DIR_VAR}/build/ardb/ardb-server",
                                "{DIR_BIN}/ardb-server")
        ...
        config = j.builder.core.file_read("{DIR_VAR}/build/ardb/ardb.conf")
        ...
        j.builder.core.file_write(cfg_path, config)
