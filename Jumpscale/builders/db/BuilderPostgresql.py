...
class BuilderPostgresql(j.builder.system._BaseClass):
    ...
    def build(self, beta=False, reset=False):
        ...
        j.builder.core.dir_ensure("{DIR_BASE}/apps/pgsql")
        j.builder.core.dir_ensure("{DIR_BIN}")
        j.builder.core.dir_ensure("$LIBDIR/postgres")
        ...
        j.builder.core.execute_bash(cmd, profile=True)
        self.doneSet('build')

    def _group_exists(self, groupname):
        return groupname in j.builder.core.file_read("/etc/group")
    ...
    def install(self, reset=False, start=False, port=5432, beta=False):
        ...
        j.builder.core.dir_ensure(self.dbdir)
        j.builder.core.execute_bash(cmd, profile=True)
